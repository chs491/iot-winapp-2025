...
- `TextBox` : 텍스트 입력을 위한 컨트롤
    ...
    - PasswordChar : 글자를 들어가는 텍스트를 암호처럼 숨길때 사용.  보통 ㅁ한자키로 특수문자 ●를 사용
    - Multiline : 여러줄 사용여부    
...
- `CheckBox` : 여러개에서 다수를 선택할 수 있는 컨트롤(라디오버튼과 동작이 반대)
    ...
    - CheckState : 
        - Unchecked : 체크를 안한 상태
        - Checked : 체크를 한 상태  
        - Intermediate : 모호한 상태(하위 체크가 일부는 체크되고 일부는 체크안됐을 때)  
    - CheckedChanged : 기본이벤트, 체크상태가 변경되면 발생    

- `TrackBar` : 사운드, 화면밝기등 최소, 최대를 스크롤로 조정하는 컨트롤    
    - Trb~ 이름 사용
    - Minimun, Maximun : ProgressBar와 동일
...
- `ListView` : 탐색기 오른쪽 목록을 표시하는데 사용하는 컨트롤
    - Lsv~
    - View : LargeIcon, SmallIcon, List, Detail 등 탐색기 보기 기능과 동일
    - SmallImageList, LrgeImageList로 아이콘 이미지 설정
    - SelectedIndexChanged : 아이템 선택이 변경될때마다 이벤트 발생

- `MonthCalendar` : 스케줄 등록을 위한 컨트롤
    - Cal~ 로 이름 사용
    - FirstDayOfWeek : 한주 시작일을 설정. default는 일요일 부터
    - DateChanged : 기본 이벤트, 날짜변경시 이벤트 발생
...
- `MenuStrip` : 프로그램 메뉴 관리
    ...
    - 메뉴 아름 작성시 (&X)와 같이 사용하면 영문자아래 밑줄 표시
    - Click : 해당메뉴 클릭 이벤트
...
- C# 고급
    1. 비동기, 스레드 - [소스](./day06/Day06Study/SyntaxWinApp01/FrmMain.cs)
        ...
        - this.Invoke, async, awailt, Task등 사용할 필요 없음
         - 취소 기능을 추가할 수 있음
...
### C# 응용 - WPF
- WPF 개요
    - `Windows Presentation Foundaation` 약자. 윈도우에서 표시하기 위한 기초.
    - 윈앱(윈폼즈)을 개선한 버전. 이미지 표현이 비트맵방식(2D). 용량문제, 계단현상 문제가 존재
    - 비트맵 방식의 문제를 해결 -> 벡터방식(텍스트). 속도개선, 용량절약

- WPF 특징
    - WPF UI 디자인에는 이미지 `벡터`방식을 사용. 비트맵 방식도 혼용가능
    - `XAML` : XML기반의 문법으로 디자인 문법으로 화면 UI를 디자인. PyQt와 동일
        - 안드로이드 등 모바일 개발, 자바 FX, Qt디자인 등 화면 UI의 최신 트렌드
    - HTML로 디자인하던 디자이너가 손쉽게 적용할 수 있음
    - 백엔드는 C# 윈앱과 거의 동일, 프론트엔드만 차이
        - 윈앱과는 다르게 코딩으로 디자인을 수행. 윈앱처럼 디자이너 사용도 가능
...
- WPF 컴포넌트(실무에서 쓰이는 UI컴포넌트) - Third Party
    ...
    - https://wpfui.lepo.co/
    - Meterial Design IN XAML Toolkit - http://materialdesigninxaml.net/
...
- 데이터바인딩 - [디자인](./day07/Day07Study/WpfStudyApp04/MainWindow.xaml), [소스](./day07/Day07Study/WpfStudyApp04/MainWindow.xaml.cs)
    ...
    - 바인딩 방식
        1. 직접코드 할당방식
            - CboCollection.ItemsSource = friutItems
            - 기존 윈앱 방식과 동일
            - 메모리에 있는 friutItems 데이터를 대입(할당)
            - DataContext와 관계 없음
            - 단순한 속성 사용방식
    ...
    - DataContext : Xaml이 데이터를 어디에서 가져오는지 설정하는 속성    
        ...
        - 바인딩 순서
            ...
            3. this.DataContext = this 로 할당 -> xaml로 전달
...
### WPF UI 프레임워크
-  개요
    - Fluent UI라 이름의 Modern UI의 한 스타일 프레임워크
...
- 기본 사용법 - [소스](./day09/Day09Study/WpfStudyApp07/MainWindow.xaml)
    ...
    - MainWindow.xaml xmls:ui 추가 App.xaml과 동일
    ```xml
